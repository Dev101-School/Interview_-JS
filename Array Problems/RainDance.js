...
function RainDance(input) {
  return input.map((currEle) => {
    return {
      name: currEle.name,
      avgRainfall:
        currEle.rainfall.reduce((acc, currIdx) => (acc = acc + currIdx), 0) /
        currEle.rainfall.length,
    };
  });
}

function MoreOptim(input) {
  return input.reduce((acc, currIdx) => {
    let currObj = {};
    currObj["name"] = currIdx.name;
    currObj["avgRainfall"] =
      currIdx.rainfall.reduce((acc, currIdx) => (acc = acc + currIdx), 0) /
      currIdx.rainfall.length;
    return [...acc, currObj];
  }, []);
}
...
console.log(RainDance(test1));
console.log(MoreOptim(test1));
